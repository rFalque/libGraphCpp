                           ...
                           graphOptions opts)
{
    ...
    std::vector<glm::vec3> polyscope_nodes;
    std::vector<std::array<size_t, 2>> polyscope_edges;

    // generate points
    for (size_t i = 0; i < nodes.rows(); i++) {
      polyscope_nodes.push_back(glm::vec3{nodes(i,0), nodes(i,1), nodes(i,2)});
    }

    // generate edges
    for (size_t i = 0; i<edges.rows(); i++) {
      std::array<size_t, 2> temp = {edges(i,0), edges(i,1)};
      polyscope_edges.push_back(temp);
    }


    // Add the curve network
    polyscope::registerCurveNetwork("my network", polyscope_nodes, polyscope_edges);
    polyscope::registerPointCloud("nodes", polyscope_nodes);
